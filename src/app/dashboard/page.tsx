...
//             <Link
//               key={canteen.id}
//               href={`/dashboard/canteens/${canteen.id}`} // Links cleanly into dynamic slug routes matching middleware configurations
...
// Define TypeScript layout matching your Prisma table configuration
interface Canteen {
  id: string;
  name: string;
...
export default function DashboardHomePage() {
  ...
  // ✨ CHANGED: Swapped hardcoded array for dynamic state array hooks
  ...
  useEffect(() => {
    ...
    // ✨ CHANGED: Fired runtime HTTP request targeting your new Prisma endpoint route
    async function loadCanteens() {
      try {
        ...
        console.error("Could not download canteens matrix:", err);
      } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-gradient-to-br from-orange-500 to-red-500 px-4 py-6 pb-8 relative overflow-hidden">
        {/* Floating Food Icons (Left) */}
        <div className="absolute left-1 top-0 h-full w-20 md:left-2 md:w-24 pointer-events-none">
          <div className="absolute left-0 top-0 opacity-70"><div className="text-xs md:text-sm mb-8 animate-[float1_4s_ease-in-out_infinite]">🍕</div></div>
          <div className="absolute left-2 top-1 opacity-60"><div className="text-xs md:text-sm mb-8 animate-[float2_5s_ease-in-out_infinite_1.5s]">🍔</div></div>
          <div className="absolute left-4 top-0 opacity-80"><div className="text-xs md:text-sm mb-8 animate-[float3_6s_ease-in-out_infinite_3s]">🍟</div></div>
        </div>

        {/* Floating Food Icons (Right) */}
        <div className="absolute right-1 top-0 h-full w-20 md:right-2 md:w-24 pointer-events-none">
          <div className="absolute right-0 top-0 opacity-70"><div className="text-xs md:text-sm mb-8 animate-[float2_5s_ease-in-out_infinite]">☕</div></div>
          <div className="absolute right-2 top-1 opacity-60"><div className="text-xs md:text-sm mb-8 animate-[float3_6s_ease-in-out_infinite_1.5s]">🍰</div></div>
        </div>
        ...
        <div className="flex flex-col items-center pt-4 relative z-10">
          ...
          <h1 className="text-2xl font-bold text-white text-center tracking-wide md:text-3xl">Campus Eats</h1>
          <p className="text-white/90 text-sm mt-1 text-center md:text-base font-medium">B.M.S. College of Engineering Canteen Center</p>
        </div>

        <style jsx>{`
          @keyframes float1 { 0% { transform: translateY(-50px); opacity: 0; } 10% { opacity: 1; } 100% { transform: translateY(200px); opacity: 0; } }
          @keyframes float2 { 0% { transform: translateY(-50px); opacity: 0; } 10% { opacity: 1; } 100% { transform: translateY(220px); opacity: 0; } }
          @keyframes float3 { 0% { transform: translateY(-50px); opacity: 0; } 10% { opacity: 1; } 100% { transform: translateY(240px); opacity: 0; } }
        `}</style>
      </header>
      ...
      <main className="p-4 pb-8 md:p-6 lg:p-8">
        <div className="mb-6">
          <h2 className="text-xl font-bold text-gray-800 mb-1 md:text-2xl">Choose Your Canteen</h2>
          <p className="text-gray-600 text-sm md:text-base">Hey {user.name}, select a campus dining hub to display their live menu options</p>
        </div>

        {/* ✨ CHANGED: Added loading layout indicator fallback blocks */}
        {canteensLoading ? (
          <div className="text-center py-8 text-gray-500 font-medium">Loading live campus stalls...</div>
        ) : canteens.length === 0 ? (
          <div className="text-center py-8 text-gray-400 bg-white border rounded-xl shadow-sm">
            No active canteens registered in the system yet.
          </div>
        ) : (
          <div className="space-y-4 md:grid md:grid-cols-2 md:gap-4 md:space-y-0 lg:grid-cols-3">
            {canteens.map((canteen) => (
              <Link
                key={canteen.id}
                href={`/dashboard/canteens/${canteen.id}`}
                className="block w-full h-28 rounded-xl overflow-hidden relative group hover:shadow-lg transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] focus:outline-none md:h-28 lg:h-32 shadow-sm border border-gray-100"
              >
                {/* Fallback image if canteen thumbnail layout block is null */}
                <div
                  className="absolute inset-0 bg-cover bg-center transition-transform duration-300 group-hover:scale-105"
                  style={{ backgroundImage: `url(${canteen.image || "https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=600"})` }}
                />
                ...
                <div className="relative h-full flex items-center justify-between px-4">
                  <div className="flex-1 text-left">
                    ...
                    <p className="text-white/80 text-xs group-hover:text-orange-100/80 transition-colors duration-200 md:text-sm">
                      {canteen.subtitle || "Welcome to our shop!"}
                    </p>
                  </div>
                  <ChevronRight className="w-5 h-5 text-white/70 group-hover:text-white group-hover:translate-x-1 transition-all duration-200 md:w-6 md:h-6" />
                </div>
