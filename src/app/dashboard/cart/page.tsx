...
import { Heart, ArrowLeft, Minus, Plus, ShoppingBag, ShieldCheck, Clock, FileText, ArrowRight, Trash2, AlertTriangle } from "lucide-react";
...
export default function CartPage() {
  ...
  useEffect(() => {
    // 1. We look up the canteenId from our first loaded cart item reference
    const activeCanteenId = cartItems[0]?.canteenId || validatedCartItems[0]?.canteenId;
    if (!activeCanteenId) return;

    // 2. Initialize the Pusher connection tunnel cleanly 
    const pusherClient = new PusherClient("4ea74b7ade3151df8b06", {
      cluster: "ap2",
    });

    const channel = pusherClient.subscribe(`canteen-${activeCanteenId}`);

    // 3. Catch the stock update broadcast and snap the cart UI state instantly
    channel.bind("menu-stock-updated", (data: { menuItemId: string; isAvailable: boolean }) => {
      console.log("📡 Cart Page: Real-time stock alert received:", data);
      
      setValidatedCartItems((prevItems) =>
        prevItems.map((item) =>
    ...
    // 4. Clean up connection parameters on route exit
    return () => {
      channel.unbind_all();
      channel.unsubscribe();
  ...
  // ✨ NEW: Explicit single item removal function for out-of-stock elements
  ...
  try {
    ...
    // Phase 1: Request an official tracking order ID from the checkout endpoint
    ...
    if (!checkoutData.success) {
      alert(checkoutData.error || "Order session initiation failed.");
      return;
    }

    // Phase 2: Configure options for the Razorpay browser window overlay modal modal
    const options = {
      key: process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || "rzp_test_TEW3NpIiGQELhn", // Frontend public key link configuration
      amount: checkoutData.amount,
      currency: checkoutData.currency,
      name: "CampusEats Gateway",
      description: `Payment review for ${canteenDetails.name}`,
      order_id: checkoutData.razorpayOrderId, // Bind the token generated from our backend route
    ...
    console.error("Verification request failed:", err);
      ...
      theme: {
        color: "#f97316" // Orange accent theme brand lines to match your existing styling definitions
      }
    };

    // Open the native payment checkout iframe window overlay
    const paymentWindow = new (window as any).Razorpay(options);
    paymentWindow.open();

  } catch (err) {
    console.error("Razorpay window execution failure:", err);
  } finally {
    setIsPlacingOrder(false);
  }
};
  ...
  return (
    <div className="min-h-screen bg-gray-100 antialiased pb-32">
      ...
      <div className="max-w-xl mx-auto p-4 space-y-4">
        {/* ✨ NEW: Bold, explicit error alert notice rendered at the top of the item review container */}
        {hasOutOfStockItems && (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-start gap-3 text-red-800 shadow-sm animate-pulse">
            <AlertTriangle className="w-5 h-5 text-red-600 flex-shrink-0 mt-0.5" />
            <div>
              <h4 className="font-black text-xs uppercase tracking-wider text-red-700">Action Required</h4>
              <p className="text-xs font-bold mt-0.5">Please remove unavailable items from your cart to place your order successfully.</p>
            </div>
          </div>
        )}

        {/* Delivery / Timing Indicator Box */}
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-start gap-3">
          <div className="w-10 h-10 bg-green-50 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5">
            <Clock className="w-5 h-5 text-green-600" />
          </div>
          ...
          <div className="divide-y divide-gray-100">
            {validatedCartItems.map((item) => (
              <div 
                key={item.id} 
                className={`p-4 flex items-center gap-4 transition-all ${
                  !item.isAvailable ? 'bg-red-50/60 border-l-4 border-l-red-500' : 'hover:bg-gray-50/30'
                }`}
              >
                {/* Image Item Thumbnail */}
                <div className="relative h-16 w-16 rounded-xl overflow-hidden bg-gray-50 border border-gray-100 flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {!item.isAvailable && (
                    <p className="text-red-700 text-[9px] font-black tracking-wider uppercase mt-1 bg-red-100 inline-block px-1.5 py-0.5 rounded">
                      OUT OF STOCK
                    </p>
                  )}
                </div>

                {/* Conditional Controller: Counter toggles vs Trash button */}
                {item.isAvailable ? (
                  <div className="flex items-center bg-orange-500 text-white rounded-lg shadow-sm border border-orange-600 overflow-hidden h-9 flex-shrink-0">
                    <button 
                      onClick={() => decreaseQuantity(item.id)} 
                  ...
                  /* ✨ NEW: Direct clean removal action button for out-of-stock entries */
                  <button
                    onClick={() => removeItemCompletely(item.id)}
                    className="p-2.5 bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 rounded-xl transition-colors flex items-center justify-center flex-shrink-0 shadow-sm active:scale-90"
        ...
        {/* Detailed billing breakdown parameters */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-50 bg-gray-50/50 flex items-center gap-2">
            <FileText className="w-4 h-4 text-gray-400" />
            <h3 className="text-xs font-black text-gray-500 uppercase tracking-wider">Bill Details</h3>
      ...
      {/* Premium Sticky Bottom Floating Action Payment processing bar block */}
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 shadow-[0_-8px_30px_rgb(0,0,0,0.04)] z-40 flex items-center justify-center">
        <div className="w-full max-w-xl flex items-center justify-between gap-4">
          <div className="text-left hidden xs:block">
            <p className="text-[10px] font-black uppercase text-gray-400 tracking-wider">Total To pay</p>
